...
export default function GetNumberOfWineByReview() {
    ...
    const { data: rest, isLoading: isLoadingRest, refetch: refetchRest } = UseRest("winery.rest", `/reviews/by_wineries?limit=${limit}&order=${order}`);
    const { data: graphQl, isLoading: isLoadingGraph, refetch: refetchGraph } = UseGrapqhQl("winery.graph", `
        {
            numberReviewsWinery(limit:${limit}, order:"${order}") {
                winery,
                numberOfReviews
            }
        }
    ...
    function displayData(loading, data) {
        return loading ?
            <CircularProgress />
            :
            <DisplayTable headers={["Winery", "Number Of Reviews"]} data={data} />
    }
    ...
    return (
        <>
            ...
            <Container maxWidth="100%" sx={{
                ...
                {
                    displayData(isLoadingGraph, graphQl?.numberReviewsWinery)
                }
            </Container>
        </>
    );
